    ...
    googleTranscribeStream(clientSocket=null, callback, erCallback){
        return client
                ...
                .on('data', data => {
                    ...
                    if (isFinal) {
                        if (clientSocket) clientSocket.emit('final-transcription', transcript)
                        if (this.logging) console.log(`google <- atocha: Final transcription: ${transcript.slice(0,40)}`)
                        callback({transcription: transcript,
...
function transcibeFile(filename, langSource, clientSocket=null, logging=false){
    return new Promise((resolve, reject) => {
        const GTranscriber = new GoogleTranscribeSession(langSource, logging)
        const writeStream2Google = GTranscriber.googleTranscribeStream(clientSocket, (finalTranscription) => {
