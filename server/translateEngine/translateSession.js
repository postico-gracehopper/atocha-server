    ...
    return this.gClient
      ...
      .on('data', (response) => {
        ...
        if (result.textTranslationResult.isFinal) {
          this.log(
            `google <- atocha: *Final* translation: ${result.textTranslationResult.translation.slice(
              0,
              40
            )}`
          );
          this.finalTranslation = result.textTranslationResult.translation;
          //   console.log('translation: ', this.finalTranslation);
          clientSocketInstance.emit(
            'final-translation',
          ...
          this.log(
            `google <- atocha: Partial translation: ${result.textTranslationResult.translation.slice(
              0,
              40
            )}`
          );
...
async function translateLocalFLAC(
  filename,
  langSrc,
  langTrgt,
  logging = false,
  ...
  var collector = ''; // Does collector do anything?
  let text = '';
