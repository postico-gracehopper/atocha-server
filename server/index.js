...
const speechToText = require('./translateEngine/textToSpeechSession');
...
io.on('connection', (socket) => {
  ...
  socket.on('audio', async (data) => {
    if (data.fileFormat === 'flac') {
      ...
      const text = await translateLocalFLAC(
        './audio/serverSaved.flac',
        data.langSource,
        data.langTarget,
        true,
      ...
      console.log('🚀 ~ file: index.js:46 ~ socket.on ~ text', text);
      /*
      speechToText('./audio/textTextToSpeech.mp3', text, data.langTarget);
      */
    }
  });
});
